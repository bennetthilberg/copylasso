...
> **Project status:** CopyLasso is in early pre-release development. The dockless app now includes first-run setup, persistent native Settings, explicit Launch at Login control, a configurable global shortcut, accessibility-aware native presentation, and one production workflow from Screen Recording permission through multi-display selection, display-snapshot-validated in-memory capture, local OCR, deterministic plain-text assembly, clipboard output, and nonactivating feedback. The final signed QA rerun has passed the complete OCR-source sweep, 1x Dell and 2x Sidecar sampling, physical cross-display clamping, appearance and keyboard-accessibility coverage, idle CPU, and ordinary-region latency. It also completed a simultaneous Time Profiler/Allocations 100-cycle run with zero leaks and a strong physical-footprint plateau; the growth row remains blocked because the protocol's separate private-memory checkpoints were not retained. Interactive cold status-item timing and a small set of explicitly classified cancellation, permission, protected-content, privacy-delta, lifecycle, persistence, and VoiceOver speech rows remain blocked or pending; the accepted stationary immediate-reuse crosshair and lock-only residuals remain documented. No public release is available.
