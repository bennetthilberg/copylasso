...
readonly release_notes="$repository_root/docs/release-notes/0.3.0.md"
...
expect_failure "only from protected main" \
    assert_protected_release_ref "refs/tags/v0.3.0-rc.1"
...
assert_release_draft_tag "v0.3.0-g55.12345"
expect_failure "draft tag name is invalid" \
    assert_release_draft_tag "v0.3.0-rc.1"
...
[[ "$(release_candidate_tag "1")" == "v0.3.0-rc.1" ]] || \
    fail "Candidate 1 must derive the exact v0.3.0-rc.1 tag."
[[ "$(release_candidate_tag "42")" == "v0.3.0-rc.42" ]] || \
    fail "Candidate 42 must derive the exact v0.3.0-rc.42 tag."
assert_release_candidate_tag "v0.3.0-rc.1"
expect_failure "release-candidate tag name is invalid" \
    assert_release_candidate_tag "v0.3.0-g55.12345"
assert_authenticated_draft_tag "v0.3.0-rc.42"
assert_authenticated_draft_tag "v0.3.0-g55.12345"
expect_failure "authenticated update draft tag is invalid" \
    assert_authenticated_draft_tag "v0.3.0"
expect_failure "authenticated update draft tag is invalid" \
    assert_authenticated_draft_tag "v0.3.0-rc.01"
...
printf '%s\n' "{
  ...
  \"draft\": true,
  \"prerelease\": true,
  \"tag_name\": \"v0.3.0-g55.12345\",
  \"target_commitish\": \"0123456789abcdef0123456789abcdef01234567\",
  \"assets\": [
    {\"name\": \"CopyLasso-0.3.0.dmg\"},
    {\"name\": \"CopyLasso-0.3.0.dmg.sha256\"},
    {\"name\": \"CopyLasso-0.3.0.dSYM.zip\"},
    {\"name\": \"CopyLasso-0.3.0-verification.zip\"}
  ]
}" > "$valid_draft"
...
expect_failure "not a draft" \
    ...
    "0123456789abcdef0123456789abcdef01234567" \
    "v0.3.0-g55.12345"
/usr/bin/sed '/CopyLasso-0.3.0.dSYM.zip/d' \
...
assert_release_workflow_assets \
    "$release_run" \
    "$release_run/CopyLasso-0.3.0-verification.zip"
/bin/rm "$release_run/CopyLasso-0.3.0.dSYM.zip"
expect_failure "protected release asset is missing" \
    assert_release_workflow_assets \
    "$release_run" \
    "$release_run/CopyLasso-0.3.0-verification.zip"
: > "$release_run/CopyLasso-0.3.0.dSYM.zip"

printf 'qualified candidate disk image\n' > "$release_run/CopyLasso-0.3.0.dmg"
(
    cd "$release_run"
    /usr/bin/shasum -a 256 CopyLasso-0.3.0.dmg > CopyLasso-0.3.0.dmg.sha256
)
...
readonly candidate_dmg_digest="$(/usr/bin/shasum -a 256 \
    "$release_run/CopyLasso-0.3.0.dmg" | /usr/bin/awk '{print $1}')"
readonly candidate_checksum_digest="$(/usr/bin/shasum -a 256 \
    "$release_run/CopyLasso-0.3.0.dmg.sha256" | /usr/bin/awk '{print $1}')"
readonly candidate_dsym_digest="$(/usr/bin/shasum -a 256 \
    "$release_run/CopyLasso-0.3.0.dSYM.zip" | /usr/bin/awk '{print $1}')"
readonly candidate_verification_digest="$(/usr/bin/shasum -a 256 \
    "$release_run/CopyLasso-0.3.0-verification.zip" | /usr/bin/awk '{print $1}')"
...
/usr/bin/jq -n \
    ...
    '{
        ...
        body: $body,
        assets: [
            {name: "CopyLasso-0.3.0.dmg", digest: $dmg_digest},
            {name: "CopyLasso-0.3.0.dmg.sha256", digest: $checksum_digest},
            {name: "CopyLasso-0.3.0.dSYM.zip", digest: $dsym_digest},
            {name: "CopyLasso-0.3.0-verification.zip", digest: $verification_digest}
        ]
    }' > "$valid_candidate"
...
/usr/bin/jq '(.assets[] | select(.name == "CopyLasso-0.3.0.dmg") | .digest) =
...
printf '%064d  %s\n' 0 CopyLasso-0.3.0.dmg > \
    "$release_run/CopyLasso-0.3.0.dmg.sha256"
readonly mismatched_checksum_digest="$(/usr/bin/shasum -a 256 \
    "$release_run/CopyLasso-0.3.0.dmg.sha256" | /usr/bin/awk '{print $1}')"
/usr/bin/jq --arg digest "sha256:$mismatched_checksum_digest" '
    (.assets[] | select(.name == "CopyLasso-0.3.0.dmg.sha256") | .digest) = $digest
...
/usr/bin/grep -Fq -- 'api repos/owner/repository/git/ref/tags/v0.3.0-rc.1' "$fake_gh_log" || \
...
if /usr/bin/grep -Fq -- \
    '--method DELETE repos/owner/repository/git/refs/tags/v0.3.0-rc.1' \
    "$fake_gh_log"; then
