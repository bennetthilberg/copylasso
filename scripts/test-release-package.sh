...
printf '%064d  CopyLasso-0.2.0.dmg\n' 0 > "$wrong_checksum"
...
expect_failure "must name CopyLasso-0.2.0.dmg" \
...
/private/example/CopyLasso-0.2.0.dmg: accepted
...
dmg_filename=CopyLasso-0.2.0.dmg
...
rm "$temporary_directory/comparison-2/CopyLasso-0.2.0.dSYM.zip"
/usr/bin/ditto -c -k --norsrc --noextattr --keepParent \
    "$comparison_dsym" "$temporary_directory/comparison-2/CopyLasso-0.2.0.dSYM.zip"
expect_failure "wrong dSYM UUID manifest" "$comparator_script" \
    "$temporary_directory/comparison-1" \
    "$temporary_directory/comparison-2"
printf 'corrupted dSYM archive\n' > \
    "$temporary_directory/comparison-2/CopyLasso-0.2.0.dSYM.zip"
