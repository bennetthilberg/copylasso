...
Success, no-text, and clipboard-failure results use one silent nonactivating HUD. A success preview collapses whitespace and contains at most 80 extended characters including its ellipsis. The preview exists only in the HUD model for its 2.5-second presentation, then is cleared. Captured pixels, complete recognized observations, the unbounded assembled string, and HUD previews never enter preferences, logs, caches, history, analytics, or telemetry.
