...
- A browser-downloaded exact candidate passes checksum, quarantine, Gatekeeper,
  drag installation, first launch, and the core capture workflow from a
  disposable local macOS user account on the maintainer's latest-stable host.
  Before download, that account must have no CopyLasso application, production
  preferences, production container, login item, or Screen Recording approval.
- The preserved macOS 14 rehearsal records browser quarantine, DMG layout,
  normal Gatekeeper launch, onboarding, permission denial and recovery,
  shortcut/menu invocation, selection, OCR, clipboard output, and HUD feedback.
- Permission denial and recovery, global-shortcut and menu invocation,
  selection focus handoff/restoration, capture, OCR, and clipboard output pass
  their release checks. At the original v0.1 publication checkpoint,
  enabled-login-item restart, ordinary reinstall, complete
  uninstall/reinstall, clone recreation, and a latest-stable guest VM remained
  explicit maintainer-accepted evidence gaps; their automated contracts and
  documented procedures remained required.
- Known limitations are documented in the release notes.
- No release-blocking defect remains open.

Evidence-scope amendment, July 17, 2026: the maintainer explicitly ended
further VirtualBuddy execution for v0.1. The partial macOS 14 rehearsal remains
factual evidence, the incomplete latest-stable attempt and unexecuted VM rows
remain accepted gaps, and the exact G30 candidate instead receives the host
browser/Gatekeeper/core smoke from the clean local-user state defined above.
This does not relax signing,
notarization, package verification, privacy, or core-workflow requirements.

Post-release qualification amendment, July 22, 2026: G33 qualifies current
public-release install, retained-preference reinstall, enabled and disabled
login-item transitions, scoped uninstall/reinstall, and fresh permission
recovery on the maintainer account. A freshly recreated disposable local macOS user
remains the documented repeatable account-isolation boundary, but the
maintainer stopped after deleting the old disposable account and before
recreation; no account-isolation, virtual-machine, APFS-clone, or additional
operating-system pass is claimed.
Real macOS 14 qualification remains a manual release gate.
A macOS 15 hosted smoke does not qualify macOS 14 behavior.
Canonical automation must continue to verify deployment target 14.0, Mach-O
minimum versions, Universal 2 output, and a maintained hosted launch
separately. Ordinary Release builds may unregister only their own generated
product beneath their approved build root; they must never unregister, replace,
or alter `/Applications/CopyLasso.app`.
