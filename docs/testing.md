...
physical cross-display clamp directions once each, idle CPU, and a zero-leak
check. One more current cross-display observation is required by the manual
matrix's three-run minimum before that row can pass.
