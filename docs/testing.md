...
G38 extends the existing `CaptureCommand`; it does not add a second permission, selection, screen-capture, clipboard, sound, feedback, lifecycle, menu-command, or shortcut pipeline. The single Capture action retains `⇧⌘2`. After one screen capture, text and code recognition begin concurrently. Any eligible supported code wins over text from the same selection; when no eligible code remains, the workflow falls back to ordinary OCR. The shared coordinator rejects overlap, and permission recovery retries the same unified Capture request.

Direct pure tests cover all five supported symbologies, exact whitespace and carriage-return/line-feed preservation, nonempty and valid-geometry filtering, unsupported and malformed observations, top-to-bottom and left-to-right visual ordering, deterministic ties, permutation stability, exact duplicate removal, one multiline payload, multiple single-line payloads, multiline ambiguity, and no-code results. Integration tests prove concurrent recognition, code-over-text precedence, text fallback, success when either recognizer alone succeeds, generic failure when neither succeeds, menu and shortcut routing, unified busy rejection and permission retry, clipboard and sound ordering, every clipboard-preserving branch, code-specific ambiguity feedback, cancellation, content release, and 100 alternating text/code results through the same action.

Real Vision tests use deterministic project-owned fixtures for QR, Code 128, Data Matrix, PDF417, and Aztec. They exercise 0°, 90°, 180°, and 270° orientation; reduced scale; reduced contrast; correctable and uncorrectable damage; multiple-code composition; exact duplicates; and no-code input. The tracked generator and audit reproduce all five PNGs byte-for-byte, pin `VNDetectBarcodesRequestRevision3`, confine Vision barcode APIs to one adapter, enforce the five-format allowlist, and reject code-path networking, persistence, logging, automatic actions, camera or file access, and entitlement growth:

```sh
./scripts/audit-code-recognition.sh
```

The complete application unit bundle also runs with networking denied. Signed UI automation and physical screen capture own the AppKit and macOS surfaces that unit tests cannot faithfully reproduce:

1. Confirm menu order is **Capture**, then the first divider. Verify its saved shortcut glyph follows the single recorder and disappears when the shortcut is cleared.
2. Open Settings and traverse the **Shortcuts** section by keyboard and VoiceOver. Confirm the single Capture recorder supports replacement, clearing, persistence across relaunch, and Debug reset. Confirm any unreleased legacy Code shortcut value is cleared and never appears in the UI.
3. Through the same menu action and saved shortcut, capture ordinary text alone and each of the five code fixtures from visible pixels. Confirm immediate crosshair and dimming, exact clipboard output, the appropriate **Copied Text** or **Copied Code** HUD, optional sound only after the write, and restoration of the prior frontmost application.
4. Select visible text together with an eligible code and confirm the exact code payload wins. Select the same text without a code and confirm OCR fallback. Capture two visually arranged unique single-line fixtures and confirm one newline in visual order. Capture the same payload twice and confirm one copy. Capture a multiline code together with a distinct code and confirm **Capture Codes Separately**, no sound, and an unchanged clipboard.
5. Capture a blank region, a too-small region, and an uncorrectable damaged fixture without readable text. Confirm no-text-or-code or cancellation feedback as applicable, no clipboard write, and no sound. Press Escape before and during drag and confirm immediate overlay cleanup with no HUD.
6. Invoke Capture again while a request is active. Confirm one selection only, immediate bounded busy feedback, and no second recognition or clipboard write. Repeat immediate mixed-result reuse for 100 approximate cycles while watching for retained overlays, cursor state, HUD replacement, growth, or failure to return idle.
7. Repeat successful text and code results with sound enabled and disabled, Settings frontmost, a full-screen application, Increased Contrast, Reduce Motion, enlarged text, and a connected second display when available. Confirm selection remains clamped to its initiating display and the HUD stays visible without activating CopyLasso.
8. Deny or revoke Screen Recording once, retry from the recovery panel, and confirm the unified request again applies code precedence and text fallback. Inspect privacy panes and confirm no Camera, Files and Folders, Accessibility, Input Monitoring, Microphone, or new authorization appears.
9. Use action-shaped payloads including an HTTPS URL, Wi-Fi syntax, contact data, and shell-like text. Confirm CopyLasso only writes inert plain text and never opens an application, network, file, or system action.
