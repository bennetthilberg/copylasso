...
G38 adds one mode-aware branch to the existing `CaptureCommand`; it does not add a second permission, selection, screen-capture, clipboard, sound, feedback, or lifecycle pipeline. Capture Text retains `⇧⌘2`. Capture Code is immediately below it in the status menu and has an independent optional shortcut that is unset by default. The shared coordinator rejects overlap between modes, and permission recovery retries the mode that initiated the request.

Direct pure tests cover all five supported symbologies, exact whitespace and carriage-return/line-feed preservation, nonempty and valid-geometry filtering, unsupported and malformed observations, top-to-bottom and left-to-right visual ordering, deterministic ties, permutation stability, exact duplicate removal, one multiline payload, multiple single-line payloads, multiline ambiguity, and no-code results. Integration tests cover menu and shortcut routing, shared Text/Code busy rejection, code-mode permission retry, clipboard and sound ordering, every clipboard-preserving branch, mode-specific feedback, cancellation, content release, and 100 alternating successful Text/Code operations.
...
1. Confirm menu order is **Capture Text**, **Capture Code**, then the first divider. Verify each saved shortcut glyph is shown beside its own command and a cleared optional Code shortcut shows no glyph.
2. Open Settings and traverse the **Shortcuts** section by keyboard and VoiceOver. Confirm independently named Text and Code recorders, replacement, clearing, persistence across relaunch, and Debug reset of both shortcuts.
3. With a simple saved Code shortcut, capture each of the five fixture formats from visible pixels. Confirm an immediate crosshair and dimming, exact clipboard payload, **Copied Code** HUD, optional sound only after the write, and restoration of the prior frontmost application.
4. Capture two visually arranged unique single-line fixtures and confirm one newline in visual order. Capture the same payload twice and confirm one copy. Capture a multiline code together with a distinct code and confirm **Capture Codes Separately**, no sound, and an unchanged clipboard.
5. Capture a blank region, a too-small region, and an uncorrectable damaged fixture. Confirm no-code or cancellation feedback as applicable, no clipboard write, and no sound. Press Escape before and during drag and confirm immediate overlay cleanup with no HUD.
6. Start Text while Code is active and Code while Text is active. Confirm one selection only, immediate bounded busy feedback, and no second recognition or clipboard write. Repeat immediate mixed-mode reuse for 100 approximate cycles while watching for retained overlays, cursor state, HUD replacement, growth, or failure to return idle.
7. Repeat a successful Code capture with sound enabled and disabled, Settings frontmost, a full-screen application, Increased Contrast, Reduce Motion, enlarged text, and a connected second display when available. Confirm selection remains clamped to its initiating display and the HUD stays visible without activating CopyLasso.
8. Deny or revoke Screen Recording once, retry from the recovery panel, and confirm Code rather than Text resumes. Inspect privacy panes and confirm no Camera, Files and Folders, Accessibility, Input Monitoring, Microphone, or new authorization appears.
...
Actual global-shortcut delivery, Screen Recording transitions, visible crosshair/dimming, physical display behavior, VoiceOver speech, audible output, and absence of automatic macOS actions remain signed manual evidence. Automation owns recognition accuracy for deterministic fixtures, mode routing, assembly, privacy isolation, error handling, lifetime, and repeatability.
