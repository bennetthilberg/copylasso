...
The July 13, 2026 signed run completed the functional, permission, OCR, privacy,
and numeric performance evidence that remained feasible without Sidecar. It then
failed pre-drag sleep/wake cleanup: after wake, the arrow was visible but an
invisible selection session still accepted a drag, changed the clipboard, and
showed success feedback. G24 is blocked at its mandatory stop condition pending
a separately approved lifecycle fix and clean rerun; Sidecar-only rows remain
blocked by unavailable hardware.
