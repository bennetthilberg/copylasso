...
The G05-G07 executable feasibility harnesses were retired after their evidence was recorded. Their former launch arguments are no longer supported. Both Debug and Release contain the production AppKit selection overlay, ScreenCaptureKit region capture, local Vision OCR, and pure text assembly, but neither contains live clipboard or feedback behavior.

The application target contains the dockless menu-bar shell, production-neutral models and service contracts, live permission and selection adapters, actor-isolated production region capture, production Vision OCR, and deterministic text assembly. Capture Text validates a selected display, captures only after overlays are absent, recognizes the in-memory image away from the main actor, and produces a transient plain string before stopping at the unimplemented G17 clipboard boundary. See [Architecture Overview](architecture/overview.md) for dependency and actor boundaries, [Plain-Text Assembly](architecture/text-assembly.md) for ordering rules, [Testing](testing.md) for signed matrices, [ADR-001](architecture/ADR-001-vision-ocr.md) for OCR evidence, [ADR-002](architecture/ADR-002-screen-capture.md) for permission and capture evidence, and [ADR-003](architecture/ADR-003-selection-overlay.md) for selection and coordinate evidence.
...
The repository contains a buildable dockless menu-bar app with onboarding, persistent Settings, Launch at Login, a configurable global shortcut, production permission recovery, multi-display selection, in-memory ScreenCaptureKit capture, local Vision OCR, pure text assembly, service test doubles, and retained feasibility evidence. The shortcut and menu both enter the same permission, selection, capture, recognition, and formatting workflow. The assembled string remains transient and is discarded before the intentionally unimplemented clipboard and feedback boundary.
