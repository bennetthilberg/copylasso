...
The canonical entrypoint verifies its own CI contract, runs the privacy/security, platform-qualification, and coverage audits, selects clean DerivedData under `.build`, verifies Xcode 26.6, lints Swift sources, resolves packages, builds Debug, builds the unit and UI test bundles, runs timeout-bounded nonparallel unit tests, repeats that already-built unit bundle with networking denied and then in three deterministic passes, asserts required settings, builds Universal 2 Release, and verifies both binary slices. It disables code signing and never launches the unsigned UI-test runner. Xcode registers an ordinary production-identifier Release build with Launch Services, so the shared scheme unregisters its generated product after ordinary Release builds. Canonical CI instead uses a failure-safe wrapper so cleanup still runs when its Release command fails. Both paths accept only the direct generated product beneath the exact DerivedData root, reject symlinks, unexpected identities, out-of-root paths, and every installed application including `/Applications/CopyLasso.app`, and skip protected archive actions. See [Automated Coverage Review](coverage-review.md) for the baseline, per-file floors, and every justified uncovered region.
...
The application target contains the dockless menu-bar shell, production-neutral models and service contracts, live permission and selection adapters, actor-isolated production region capture, production Vision text and code recognition, deterministic assemblers, and the output adapters. Capture validates a selected display, captures only after overlays are absent, recognizes text and supported codes from the same in-memory image away from the main actor, gives eligible codes precedence over OCR fallback, produces one transient plain string, writes nonempty content to the general pasteboard, and presents bounded feedback without activation. See [Architecture Overview](architecture/overview.md) for dependency and actor boundaries, [Capture Workflow](architecture/capture-workflow.md) for the complete operation and lifetime contract, [Plain-Text Assembly](architecture/text-assembly.md) for OCR ordering rules, [Clipboard and Feedback](architecture/clipboard-and-feedback.md) for output privacy/lifetime rules, [Security and Privacy Review](security-and-privacy-review.md) for entitlements, trust boundaries, and dependency evidence, [Testing](testing.md) for signed matrices, [ADR-001](architecture/ADR-001-vision-ocr.md) for OCR evidence, [ADR-002](architecture/ADR-002-screen-capture.md) for permission and capture evidence, and [ADR-003](architecture/ADR-003-selection-overlay.md) for selection and coordinate evidence.

## GitHub Actions

`.github/workflows/ci.yml` runs for pull requests targeting `main` and pushes to `main`. It explicitly selects Xcode 26.6 and executes `scripts/ci.sh`, including its three-pass repeatability gate, on GitHub-hosted macOS 26 Apple Silicon and Intel runner images. The arm64 job transfers its exact Universal 2 Release artifact to a separate GitHub-hosted macOS 15 arm64 job, which verifies macOS 14.0 deployment metadata and both Mach-O minimum versions, ad-hoc signs the app on that host, and proves a clean process launch without invoking protected resources. The workflow has read-only repository contents permission, persists no checkout credential, uses no secrets or cache, and bounds concurrent runs, artifact retention, and job duration.

The matrix reports `build and test (arm64)`, `build and test (x86_64)`, and `maintained OS runtime (macOS 15 arm64)`. Once the new maintained-runtime context has passed on the migration pull request and no predecessor pull request remains open, branch protection must atomically replace the retired minimum-OS context before the migration can merge; the two architecture requirements and strict up-to-date policy remain unchanged. Maintainers can apply the temporary `ci-failure-probe` label to a pull request to compile a controlled unit-test failure into both build jobs. Removing the label restores all checks at the same commit. Delete the temporary repository label after verifying both transitions.

GitHub began deprecating its hosted macOS 14 images on July 6, 2026 and announced their removal for November 2, 2026. G33 therefore moved continuous artifact launch coverage to the maintained macOS 15 image while preserving the deployment metadata checks. A macOS 15 hosted smoke does not qualify macOS 14 behavior. Real macOS 14 qualification remains a manual release gate, backed by the preserved signed Sonoma evidence and repeated on real Sonoma before a future release when relevant code or packaging changes. The pinned KeyboardShortcuts 3.0.1 manifest still requires Swift tools 6.2, so source compilation remains on the selected Xcode 26.6 runners rather than an older host toolchain.
