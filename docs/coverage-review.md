...
| --- | --- | --- |
| SwiftUI `SettingsView`, `OCRLanguageEditorView`, `AboutView`, onboarding, status, and menu builders | Instantiating declarative builder branches without a window does not prove layout, focus, keyboard order, or accessibility. The aggregate explicitly excludes the three retained-state-dependent onboarding builders and G51's sheet-only language editor; their pure state and controller branches remain measured separately. | Signed focused XCUITests plus the G21/G24/G51 manual accessibility matrices. UI tests have no unconditional retry and are not reported as hosted passes when the runner is unsigned or shielded by `loginwindow`. |
