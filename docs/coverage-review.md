...
| --- | --- | --- |
| SwiftUI `SettingsView`, `OCRLanguageEditorView`, `CaptureHistoryView`, `AboutView`, onboarding, status, and menu builders | Instantiating declarative builder branches without a window does not prove layout, focus, keyboard order, or accessibility. The aggregate explicitly excludes the retained-state-dependent onboarding builders, G51's sheet-only language editor, and G52's window-only history view; their pure state and controller branches remain measured separately. | Signed focused XCUITests plus the G21/G24/G51/G52 manual accessibility matrices. UI tests have no unconditional retry and are not reported as hosted passes when the runner is unsigned or shielded by `loginwindow`. |
