...
| Selection and capture | User-controlled rectangle event coordinates, then one ScreenCaptureKit `CGImage` | Observe-only transition monitor and private async operation scope | None; no encoded subprocess image, file, or screenshot-clipboard output exists |
...
| Passive selection tracking | While the native selector is active, an AppKit global monitor receives observe-only copies of left-button-down, drag, and left-button-up events and retains only their Core Graphics coordinates plus Control and Space state. It intercepts no event, synthesizes no input, requests no Accessibility or Input Monitoring access, and discards the transitions when selection ends. A click without a drag cannot become selection geometry. |
| Control-modified native selection | macOS can redirect the native selector to its screenshot clipboard when Control is held. CopyLasso rejects Control before launch, polls it while selection is active, records it from each copied pointer event including mouse-up, and interrupts the child cleanly if it appears. It never reads or snapshots the pasteboard. |
| Space-adjusted native selection | Holding Space while dragging can reposition the selector's visible rectangle so its final bounds differ from the original endpoints. CopyLasso detects Space on dragged pointer events and cancels instead of passing uncertain geometry to ScreenCaptureKit. |
| Cross-display native selection | The system selector can display a rectangle spanning displays, while CopyLasso captures one display at a time. A release on a different known display cancels instead of silently cropping the visible rectangle. |
