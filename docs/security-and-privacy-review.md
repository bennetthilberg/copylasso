...
| Passive selection tracking | While the native selector is active, an AppKit global monitor receives observe-only copies of left-button-down and left-button-up events and retains only their Core Graphics coordinates; CopyLasso separately checks the Control modifier. It intercepts no event, synthesizes no input, requests no Accessibility or Input Monitoring access, and discards the transitions when selection ends. |
