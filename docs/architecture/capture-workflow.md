...
- While that selector runs with its image destination fixed to `/dev/null`, CopyLasso uses an observe-only global mouse monitor to retain the exact left-button-down, drag, and left-button-up event coordinates. A press/release pair without a drag, including a direct-access confirmation click, cannot become selection geometry; if more than one complete drag arrives, only the final candidate can be accepted when the selector exits. A release on a different known display cancels instead of silently cropping the rectangle the native selector displayed. Valid same-display geometry is clamped to its initiating display and passed to the existing ScreenCaptureKit adapter, which revalidates the complete display bounds before returning selected pixels in memory. No subprocess image bytes, screenshot file, event interception, or screenshot-pasteboard value enter CopyLasso.
- Control is rejected before selector launch, polled while it runs, and recorded from each copied pointer event, including the decisive mouse-up. If it appears, the child is stopped and the attempt fails closed. A drag event observed while Space is held also stops the attempt because macOS can reposition the visible rectangle in a way that cannot be reconstructed from its endpoints. These guards require no Accessibility or Input Monitoring permission and end with the selector session.
