...
G18 connects the menu and global shortcut to one production operation without placing private content in observable application state. G38 adds a mode decision inside that same operation so Capture Text and Capture Code share every permission, selection, capture, output, lifecycle, and busy-state boundary.
...
flowchart LR
  ...
  Mode --> OCR["Local text OCR"]
  Mode --> Barcode["Local code recognition"]
  OCR --> Assembly["Pure text assembly"]
  Barcode --> CodeAssembly["Pure payload assembly"]
  CodeAssembly --> Clipboard
...
`CaptureCoordinator` stores only a payload-free phase. `CaptureCommand` is the single root-owned invocation boundary used by both menu commands and both optional global shortcuts. A request received in any non-idle phase is rejected without creating another overlay, capture, recognition request, clipboard write, sound, or HUD. Permission recovery retains only the initiating mode and retries that mode.

## Capture Code

`VisionBarcodeService` is the only production Vision barcode adapter. It pins `VNDetectBarcodesRequestRevision3` and requests only QR, Code 128, Data Matrix, PDF417, and Aztec. It performs recognition in a cancellable detached task and converts Vision results to neutral observations before returning to the shared workflow.
...
After selection, one private async function owns the selected geometry, captured `CGImage`, neutral text or code observations, and full assembled string. The geometry carries the initiating display point size and scale; the capture adapter compares them with a fresh ScreenCaptureKit display snapshot before requesting pixels. The function returns only:

- a mode-specific no-result or code-ambiguity value; or
- a mode-specific success with a whitespace-normalized preview bounded to 80 extended grapheme clusters after the full string has been written.
...
- Success, no-text, no-code, and code-ambiguity enter `completing` only while presenting feedback, then return immediately to idle while the feedback panel owns its independent dismissal timer. A new idle capture may therefore dismiss visible feedback and begin selection without waiting for that timer.
...
The canonical suite injects every service, exercises all branch classes, performs 25 consecutive successful operations, 20 alternating success/cancel cycles, and 100 alternating Text/Code operations, rejects concurrent work across modes, proves each menu and shortcut route uses the same command, and cancels pending selection, capture, recognition, sound, and feedback work. Real Vision tests cover all five symbologies, rotations, degradation, damage, compositions, and duplicates with deterministic project fixtures. The signed manual matrix remains necessary for arbitrary app pixels, full-screen Spaces, real paste targets, physical displays, sleep/wake, lock/unlock, and actual audio-output behavior.
