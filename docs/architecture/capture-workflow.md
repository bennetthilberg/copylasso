...
- While that selector runs with its image destination fixed to `/dev/null`, CopyLasso samples only the current pointer position and left-button state at one-millisecond intervals. The tracker waits out a button already held when the selector starts, treats each later press/release pair as a candidate, and retains the final completed candidate when the child exits. A tiny direct-access confirmation click therefore cannot prevent the later real drag. A release on a different known display cancels instead of silently cropping the rectangle the native selector displayed. Valid same-display geometry is clamped to its initiating display and passed to the existing ScreenCaptureKit adapter, which revalidates the complete display bounds before returning selected pixels in memory. No subprocess image bytes, screenshot file, event monitor, event interception, or screenshot-pasteboard value enter CopyLasso.
- Control is rejected before selector launch and polled while it runs. If it appears, the child is stopped and the attempt fails closed. This guard requires no Accessibility or Input Monitoring permission and ends with the selector session.
