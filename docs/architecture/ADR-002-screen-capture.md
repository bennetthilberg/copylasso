...
The service rejects missing or reconfigured displays, nil output, and unexpected image dimensions. `SCStreamError.userDeclined` maps to authoritative permission denial; the permission history records previously observed access, an in-process denial flag prevents stale preflight from re-entering capture before relaunch, and the existing singleton recovery panel is presented. No raw framework error, pixel content, or application identity enters coordinator state or logs. The returned `CGImage` is forwarded directly to the G15 Vision service and released after recognition or cancellation.
