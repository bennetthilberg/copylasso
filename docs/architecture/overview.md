...
- The production interactive-capture service starts the system selector synchronously after granted shortcut preflight, so macOS owns cursor and focus behavior. It derives one display-clamped rectangle from transient pointer state, receives no subprocess image output, and suppresses stale completion after cancellation or replacement. The ScreenCaptureKit adapter then validates current display identity, dimensions, scale, and bounds before returning only those pixels in memory.
