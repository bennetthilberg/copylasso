...
- `Services` declares narrow permission, interactive capture, Debug selection/capture, text recognition, code recognition, clipboard, sound, and feedback boundaries. Production starts only the fixed `/usr/sbin/screencapture -i -s -x -t png /dev/stdout` contract directly through `Process`; no shell or caller-controlled argument enters that boundary. The AppKit and ScreenCaptureKit adapters remain available only to explicit Debug fixture modes.
...
- The production interactive-capture service starts the system selector synchronously after granted shortcut preflight, so macOS owns cursor and focus behavior. It streams stdout into a 128 MiB in-memory ceiling, accepts one valid PNG within a 100-million-pixel ceiling, decodes one `CGImage`, and suppresses stale completion after cancellation or replacement. Empty successful or Escape output is an inert cancellation.
- The AppKit selection and ScreenCaptureKit capture adapters remain deterministic Debug boundaries for overlay, geometry, and pixel-crop regression coverage. Normal Debug runs and Release do not wire them into the Capture command.
