...
The application is a dockless SwiftUI menu-bar utility. Debug and Release compile the same onboarding, Settings, Launch at Login, global-shortcut, model, service-contract, capture-workflow, production permission, recovery-panel, and AppKit selection code. Neither configuration contains a pixel-capture, OCR, clipboard, or feedback adapter.
...
There is no source-file exclusion list for experimental code. The G05-G07 executable experiments were retired after their decisions were recorded. The canonical pipeline rejects their former launch arguments and every deferred ScreenCaptureKit, Vision, legacy Core Graphics capture, window-sharing exclusion, and pasteboard API. It permits the two Core Graphics Screen Recording authorization functions only in `ScreenCapturePermissionService.swift` and the display/panel/cursor APIs only in `AppKitRegionSelectionService.swift`; enforces model/workflow import boundaries; rejects Debug-only G12-G13 controls in Release; and verifies that the coordinator, geometry model, production permission service, recovery controller, production selection service, and no-pixel G14 boundary compile into both Debug and Release.
