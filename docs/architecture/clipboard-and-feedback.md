...
The player loads the original bundled `CopyLassoSuccess.wav` through `NSSound`, restarts the same short sound from the beginning for rapid successful captures, and returns immediately. G40A refines only that deterministic asset's timbre; it does not change this playback boundary or any invocation policy. A missing or unreadable asset, muted system output, unavailable output device, or playback refusal produces no additional UI, error, focus change, permission request, or capture delay. Lifecycle cancellation and termination stop any transient playback.
