...
- **Implementation status:** Released as 0.2.0 (3) on July 29, 2026. The secure updater, configurable success sound, and unified on-screen code recognition ship in v0.2. G39 concluded no-go for offline LaTeX recognition, so no LaTeX implementation is present.
...
0.2 delta and the promises that remain in force for public version 0.2.0.
...
CopyLasso is publicly available as version and build `0.2.0 (3)`. The secure
...
Current unreleased source replaces the public 0.2.0 selection surface with
...
accepted for this unreleased source: exact Control at mouse-up can let macOS
...
G39 concluded no-go, so CopyLasso 0.2.0 contains no LaTeX recognition, model,
