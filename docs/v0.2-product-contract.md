...
- **Implementation status:** Release-qualified in source as 0.2.0 (3); not publicly released. The secure updater, configurable success sound, and unified on-screen code recognition are frozen for v0.2. G39 concluded no-go for offline LaTeX recognition, so no LaTeX implementation is present.
...
CopyLasso remains publicly available as version and build `0.1.1 (2)`.
G41 freezes the qualified source at `0.2.0 (3)` after the actual v0.2 feature
set passes release-level qualification. This source status does not publish a
feed, tag, release, or artifact.

The secure updater, configurable success sound, and unified on-screen code recognition are implemented in source but are not part of the public CopyLasso 0.1.1 download.
