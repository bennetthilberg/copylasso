...
- **Implementation status:** Released as 0.2.1 (4) on August 9, 2026. The secure updater, configurable success sound, unified on-screen code recognition, bounded update-note presentation, and focus-preserving native selector ship in v0.2. G39 concluded no-go for offline LaTeX recognition, so no LaTeX implementation is present.

Candidate source `0.2.2 (5)` changes only the pinned updater framework to its
security-fixed release. Public behavior, permissions, entitlements, and
...
0.2 delta and the promises that remain in force for public version 0.2.1.
...
CopyLasso is publicly available as version and build `0.2.1 (4)`. The secure
...
Public 0.2.1 replaces the original public 0.2.0 selection surface with
macOS's fixed interactive selector, without activating CopyLasso or changing
the frontmost application. Shift, Option, and Control in the saved shortcut
must be released before the selector starts. CopyLasso then samples pointer and
button state every millisecond, derives one initiating-display rectangle, and
captures only that rectangle through the existing in-memory ScreenCaptureKit
boundary.
...
G39 concluded no-go, so CopyLasso 0.2.1 contains no LaTeX recognition, model,
