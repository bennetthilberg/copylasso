...
- **Implementation status:** Released as 0.2.2 (5) on August 10, 2026. The secure updater, configurable success sound, unified on-screen code recognition, bounded update-note presentation, and focus-preserving native selector ship in v0.2. G39 concluded no-go for offline LaTeX recognition, so no LaTeX implementation is present.

Public 0.2.2 changes only the pinned updater framework to its security-fixed
release. Public behavior, permissions, entitlements, and the
full-package-only update policy remain unchanged.
...
The historical [v0.1 product contract](v0.1-product-contract.md) remains the
source of truth for the 0.1 release line. This contract describes the shipped
0.2 delta and the promises that remain in force for public version 0.2.2.
...
The existing Capture workflow remains stable:

- `Shift-Command-2` (`⇧⌘2`) remains assigned to the single Capture command.
- Capture still selects visible screen pixels, recognizes supported codes and
  ordinary horizontal English text locally with Vision, and writes successful
...
CopyLasso is publicly available as version and build `0.2.2 (5)`. The secure
updater, configurable success sound, and unified on-screen code recognition all
ship in that release. The immutable release and authenticated feed are recorded
in the [v0.2 release-state record](v0.2-release-state.md).

### Native-selector maintenance amendment

Public 0.2.2 retains the 0.2.1 replacement of the original public 0.2.0
selection surface with macOS's fixed interactive selector, without activating
CopyLasso or changing the frontmost application. Shift, Option, and Control in
the saved shortcut must be released before the selector starts. CopyLasso then
samples pointer and button state every millisecond, derives one
initiating-display rectangle, and captures only that rectangle through the
existing in-memory ScreenCaptureKit boundary.

This focus-preserving design deliberately adds no Accessibility or Input
Monitoring permission and does not intercept input. Three narrow residuals are
accepted for this selector: exact Control at mouse-up can let macOS
replace the clipboard with its screenshot before CopyLasso cancels; dragging
across the first-use direct-access confirmation and then pressing Escape can be
mistaken for a completed selection; and an entire exceptionally fast drag
between samples can be missed or begin at the first observed point. CopyLasso
never reads a screenshot clipboard value or continues it to recognition. These
exceptions do not weaken cancellation or clipboard preservation for ordinary
direct drags, Escape, permission denial, or CopyLasso-originated failures.

## Recognition Command and Shortcut

CopyLasso exposes one Capture command and one configurable shortcut. The
application determines the result from the selected pixels without making the
user choose a recognizer first.

- Capture retains `⇧⌘2` and the existing persisted Capture Text shortcut key so
  upgrades do not discard a user's configured shortcut.
- Text and code recognition begin concurrently after the single in-memory
  screen capture.
- Any eligible supported code result wins over recognized text, including when
  the same selection contains both. Multiple-code ordering, exact duplicate
  removal, and multiline ambiguity protection remain authoritative.
- If no eligible code remains, CopyLasso uses the ordinary OCR result.
- If neither recognizer produces eligible content, CopyLasso preserves the
  clipboard and presents one bounded no-text-or-code result.
- A successful result from either recognizer remains usable when the other
  recognizer fails. If neither succeeds, CopyLasso presents one generic
  recognition failure and preserves the clipboard.
- Starting Capture while permission, selection, capture, or recognition is
  already active follows the existing single-workflow busy contract.

G39 concluded no-go, so CopyLasso 0.2.2 contains no LaTeX recognition, model,
runtime, command, or precedence branch. G40 is omitted. A future LaTeX proposal
requires a new product-contract and roadmap decision.
G39 recommends no-go for offline LaTeX recognition, and that accepted decision
remains release-blocking for v0.2.

## Secure Update Contract

Version 0.2.0 is the first updater-enabled release and requires one manual
download by users of 0.1.x. Installing it does not retroactively add an updater
to the already published 0.1.x binaries, but it enables authenticated checks
for later releases.
...
- Staged update data is bounded, app-owned, and removed after success,
  ...
  user content.
- G35 established the dependency decision, threat model, signing-key
  separation, static feed location, and local malicious-fixture proof. G36
  integrated the accessible update workflow and qualified a private staged
  upgrade. G43 published the separately approved immutable feed and release;
  G50 later published the exact security-fixed 0.2.2 package and feed without
  changing this contract.

The updater is the only approved v0.2 network client. Its entitlement and
implementation remain confined to the design proved by G35 and shipped by G36.
...
- If multiple unique payloads remain and any contains a line break, CopyLasso
  preserves the clipboard and presents a bounded, code-specific ambiguity
...
- Different payloads are never merged, interpreted, reformatted as actions, or
  silently dropped.
- If no eligible payload remains, CopyLasso falls back to ordinary text OCR.
  Only a selection with neither eligible code nor recognized text presents the
  bounded no-text-or-code result.
...
A G39 study applies the gates sequentially. Licensing and provenance, complete
installed size, maintained macOS 14 support for both architectures,
offline/sandbox feasibility, and access to the required physical systems are
preblind gates. If no candidate survives them, G39 records no-go without
creating or unsealing the blind corpus. That early stop preserves the corpus
and does not waive any gate for a later candidate.
...
- The updater persists only the minimum non-content state required for check
  timing, deferral, staged authenticated bytes, and recovery. G35 defined the
...
- No new capture mode may add a privacy permission. The success sound requests
  ...
  only v0.2 entitlement change.
- Core text and code capture remain fully usable while
